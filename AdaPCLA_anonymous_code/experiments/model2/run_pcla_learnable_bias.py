...
and use a **smaller learning rate for the bias** than for other parameters.

与 model1 中的版本相比，本脚本唯一关键差异：
- 优化器使用参数组，将 `self.output_bias` 的学习率设置为其他参数的 0.01 倍。
"""
...
def run_single_seed(args: Args) -> None:
    ...
    # --------- 关键改动：为 bias 使用更小的学习率 ---------
    base_lr = cfg.lr
    bias_lr = base_lr * 0.01
    bias_params = []
    other_params = []
    ...
    assert bias_params, "No parameter containing 'output_bias' found in model.named_parameters()."
    ...
    print(f"Optimizer configured with lr={base_lr} for main params, lr={bias_lr} for output_bias.")
    ...
    print("Start synthetic generation with learnable-bias PCLA (separate LR for bias) ...")
    model.eval()
    ...
    print(f"Saved synthetic dataset (learnable bias, sep LR): {out_path} (n={len(generated)})")
    ...
    if args.do_eval:
        ...
        print(
            f"[LearnableBias PCLA (sep LR) seed={args.seed}] mean_acc={mean_acc_lb:.6f}, "
        ...
        # 写对比 CSV / JSON
        out_csv = os.path.join(OUTPUT_ROOT, "pcla_fixed_vs_learnable_bias_sepLR_summary.csv")
        with open(out_csv, "w", newline="") as f:
            ...
            w.writerow(
                [
                    f"PCLA_learnable_bias_sepLR_seed{args.seed}",
                    f"{mean_acc_lb:.6f}",
                    "0.000000",  # 单次运行，std 记为 0
                    f"{mean_f1_lb:.6f}",
                    "0.000000",
                    "This run: single-seed PCLA with learnable output bias, smaller LR for bias (0.01x)",
                ]
            )

        out_json = os.path.join(OUTPUT_ROOT, "pcla_fixed_vs_learnable_bias_sepLR_summary.json")
        with open(out_json, "w", encoding="utf-8") as f:
            json.dump(
                {
                    "learnable_bias_sepLR": {
                        "seed": args.seed,
